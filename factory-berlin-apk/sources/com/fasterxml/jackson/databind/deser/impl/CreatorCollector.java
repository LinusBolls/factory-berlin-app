            ...
            if (settableBeanPropertyArr.length > 1) {
                ...
                for (int i2 = 0; i2 < length; i2++) {
                    ...
                    if ((!name.isEmpty() || settableBeanPropertyArr[i2].getInjectableValueId() == null) && (num = (Integer) hashMap.put(name, Integer.valueOf(i2))) != null) {
                        throw new IllegalArgumentException(String.format("Duplicate creator property \"%s\" (index %s vs %d) for type %s ", new Object[]{name, num, Integer.valueOf(i2), ClassUtil.nameOf(this._beanDesc.getBeanClass())}));
                    }
                }
            }
